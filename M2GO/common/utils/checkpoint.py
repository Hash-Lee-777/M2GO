...
class Checkpointer(object):
    ...
    def load(self, path=None, resume=True, resume_states=True):
        ...
        model_to_load = self.model.module if isinstance(self.model, (DataParallel, DistributedDataParallel)) else self.model
        ...
        if skipped_unexpected:
            self._print('Ignoring {} unexpected keys from checkpoint: {}'.format(len(skipped_unexpected), skipped_unexpected))
        if skipped_mismatch:
            mismatch_msg = ['{} (ckpt {}, model {})'.format(k, ckpt_shape, model_shape)
                            for k, ckpt_shape, model_shape in skipped_mismatch]
            self._print('Ignoring {} keys due to shape mismatch: {}'.format(len(skipped_mismatch), mismatch_msg))

        incompatible = model_to_load.load_state_dict(compatible_state, strict=False)
        missing_keys = getattr(incompatible, 'missing_keys', [])
        unexpected_keys = getattr(incompatible, 'unexpected_keys', [])
        if missing_keys:
            self._print('Missing keys after load (will be randomly initialized): {}'.format(missing_keys))
        if unexpected_keys:
            self._print('Unexpected keys after load: {}'.format(unexpected_keys))
        if resume_states:
