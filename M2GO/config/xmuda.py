...
from M2GO.common.config.base import CN, _C
...
# ----------------------------------------------------------------------------- #
# Confidence / CosMix options (optional)
# ----------------------------------------------------------------------------- #
_C.CONFIDENCE = CN()
_C.CONFIDENCE.ENABLE = False
_C.CONFIDENCE.TYPE = 'fixed'   # fixed | quantile
_C.CONFIDENCE.TAU = 0.95       # used when TYPE == fixed
_C.CONFIDENCE.Q = 0.8          # used when TYPE == quantile
_C.CONFIDENCE.LOG_EVERY = 500

_C.COSMIX = CN()
_C.COSMIX.ENABLE = False
# If True, replace target XM KL(P2D||P3D) / KL(P3D||P2D) with KL(P2D||Pmix) / KL(P3D||Pmix)
_C.COSMIX.REPLACE_XM = True
_C.COSMIX.LAMBDA = 1.0
_C.COSMIX.GATE_BY_CONF = True
# numerical / stability
_C.COSMIX.EPS = 1e-6
_C.COSMIX.ALPHA_CLAMP = (0.05, 0.95)
# ---------------------------------------------------------------------------- #
# Misc options
# ---------------------------------------------------------------------------- #
