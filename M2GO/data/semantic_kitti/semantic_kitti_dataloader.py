...
class SemanticKITTIBase(Dataset):
    ...
    def __init__(self,
        ...
        self.domain = domain
        self.unknown_classes = set(unknown_classes or [])
        self._is_source_train = bool(
            self.openset and (self.domain == 'source') and any('train' in s for s in split)
        )
        ...
        if merge_classes:
            ...
            unknown_set = self.unknown_classes
            for cat_idx, cat_list in enumerate(self.categories.values()):
                for class_name in cat_list:
                    ...
                    if self._is_source_train and class_name in unknown_set:
                        continue
                    self.label_mapping[class_id] = cat_idx
            self.class_names = list(self.categories.keys())
        else:
            self.label_mapping = None

        # build fine-class metadata for open-set evaluation
        ordered_fine = sorted(self.class_name_to_id.items(), key=lambda kv: kv[1])
        self.fine_class_names = [name for name, _ in ordered_fine]
        max_fine_id = ordered_fine[-1][1]
        self._fine_label_remap = -np.ones(max_fine_id + 1, dtype=np.int64)
        for idx, (_, class_id) in enumerate(ordered_fine):
            self._fine_label_remap[class_id] = idx
...
class SemanticKITTISCN(SemanticKITTIBase):
    ...
    def __getitem__(self, index):
        ...
        points = data_dict['points'].astype(np.float32, copy=True)
        seg_label_fine = data_dict['seg_labels'].astype(np.int64)
        seg_label_fine_idx = self._fine_label_remap[seg_label_fine]
        # replace unmapped ids (if any) with 0 to avoid negative indices downstream
        invalid_mask = seg_label_fine_idx < 0
        if invalid_mask.any():
            seg_label_fine_idx[invalid_mask] = 0
        seg_label = seg_label_fine.copy()
        ...
        if self.bottom_crop:
            ...
            seg_label_fine_idx = seg_label_fine_idx[keep_idx]
        ...
        if self.output_orig:
            out_dict.update({
                'orig_seg_label': torch.from_numpy(seg_label.copy()).long(),
                'orig_seg_label_fine': torch.from_numpy(seg_label_fine_idx.copy()).long(),
                'orig_points_idx': torch.from_numpy(idxs.astype(np.bool_)),
            })
...
def test_SemanticKITTISCN():
    from M2GO.data.utils.visualize import draw_points_image_labels, draw_bird_eye_view
    ...
    # pselab_paths = ("/home/docker_user/workspace/outputs/M2GO/a2d2_semantic_kitti/M2GO_crop_resize/pselab_data/train.npy",)
    # split = ('train',)
    split = ('val',)
